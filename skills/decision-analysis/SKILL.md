...
- No defensible prior or likelihood ratio — the evidence is qualitative, or an `UNRESOLVED` ledger supplies none: Evidence and update carries no update, because § Numeric Policy bars a `sensitivity-only` number there.
  Its Prior odds and Posterior odds slots read `none supported — see Robustness`, its Evidence table lists each item with `none supported` in the LR column and the reason beside it, and the block says in words that no supported update exists.
  Robustness does the work: sweep the prior class the losses make relevant as `sensitivity-only`, and report where the action flips.
  The verdict is `prior-sensitive` with that crossover, or `dominated` when one action wins under every state; the Recommended action slot follows § Verdict's rule for whichever it is, and "the evidence in hand does not quantify this" is a legitimate thing for the Conditions slot to say.
  Worked example: a ledger leaves "the March drop was the pricing change" `UNRESOLVED` on two non-discriminating tests; the record maps that row to the proposition, writes `Prior odds: none supported — see Robustness`, lists the two tests as evidence items with `none supported (non-discriminating)` as their LR, sweeps priors 1:4 to 4:1 as `sensitivity-only` against a user-elicited loss ratio of 3, finds the flip at 3:1, and returns `prior-sensitive — crossover at 3:1; no supported update exists` to the owner.
