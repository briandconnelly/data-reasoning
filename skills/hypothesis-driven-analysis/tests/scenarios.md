...
- [ ] The no-difference result on the median claim is recorded as `NON_DISCRIMINATING`, not as refutation of the regression, with the sensitivity stated in the form the branch that ran supplies: when the interval form ran, the interval at the claim's grain read against the predicted ~230ms shift suffices; when the fallback branch ran, the detection limit must be named.
