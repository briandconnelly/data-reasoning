...
- **CS8.5** — for exclusion, and for monotonicity when CS8.2 requires it, the evidential state is one of the two the ground-truth file allows: not testable from this extract, with the checks considered, why each is silent, and the data that would make it testable; or not contradicted by an implication the ground-truth file names — the outcome-distribution bound, stated as following from independence, exclusion, and monotonicity together, or for monotonicity the sign of the first stage — with what it cannot see named. The placebo, the balance checks, or the first stage's size given as evidence for either assumption fails, and so does a statement that any check supports, confirms, or establishes either.
...
- **CS8.8** — post only, recorded, decides no row: `check_review.py` exits 0 on the record, and the exclusion and monotonicity rows each read either probe `NONE` with assessment `not-testable-here`, or a named implication with assessment `not-contradicted`.
...
One post arm (`canary-cs8-post`) runs first and is scored on rationale: did it reach one of the two records the ground-truth file allows for exclusion and for monotonicity because it read and applied the changed text — the distinction between a weak probe and a silent check — and did it run the probes the extract plainly feeds rather than reaching for the untestable value?
