...
CS8.5 and CS8.8 were tightened for monotonicity after the close, on the PR #50 reviews, so that this document and the catalog state one contract; the design-review file records why.
...
What the extract does leave is a joint bound: if the invitation is independent of outcomes, reaches late payments only through enrollment, and moves no one's enrollment the wrong way, the two arms' outcome distributions differ by no more than the first-stage difference. The distance is 0.079 against a first stage of 0.304 (validator trap 7, seen to fire on a planted direct effect), so the bound holds with room and only a very large violation could break it.
...
Monotonicity's observable implication is the sign of the first stage, which CS8.3 already has every passing record run; so its one sound record is not contradicted by that sign, with the blind spot named.
The ground-truth file therefore allows two sound records for exclusion — not contradicted by the bound with its blind spot named, or not testable here with the checks considered and why each is silent — one for monotonicity, and one disposition.
...
- CS8 post: exclusion either `not-testable-here` with probe `NONE` and the placebo named as a check considered and found silent, or `not-contradicted` by the outcome bound; monotonicity `not-contradicted` by the first-stage sign; relevance and independence `not-contradicted`; disposition `identified-if` with both conditions named, carried into Handoff.
...
- **Monotonicity.** CS8.2, CS8.5, CS8.6, and CS8.7 key on it when the block's estimand is an effect among compliers. An arm that instead states the estimand as the intent-to-treat effect of the invitation, in this block or a second one, owes the alternative restriction it relies on, and loses nothing for the choice.

## Canary rule (PROTOCOL step 4)

One post arm (`canary-cs8-post`) runs first and is scored on rationale: did it reach a record the ground-truth file allows for exclusion and for monotonicity because it read and applied the changed text — the distinction between a weak probe and a silent check — and did it run the probes the extract plainly feeds rather than reaching for the untestable value?
