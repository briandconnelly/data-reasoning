...
`not-testable-here` is a finding a reader can contest, not an exit: an implication the review can name owes its probe, however weak — a bound only a large violation would break is a probe, assessed with that blind spot named — and a check that tests several assumptions jointly is a probe of each of them.
