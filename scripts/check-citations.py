...
def in_scope(path: Path) -> bool:
    # DEFAULT_SCOPE entries are repo-relative directory paths, which may be
    # several segments deep (`skills/hypothesis-driven-analysis`). Comparing
    # rel.parts[0] against them silently matched nothing after the move to a
    # plugin layout, and an empty match here is indistinguishable from a clean
    # pass: every file gets filtered out and the check exits 0 having read
    # nothing.
    try:
